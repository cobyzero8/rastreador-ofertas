...
def motor_ripley(url, limite, headers=None):
    ...
    # 1. Aseguramos usar la URL de búsqueda estándar de Ripley
    url_ripley = "https://simple.ripley.com.pe/search/televisor?sort=relevance_desc&page=1"
    
    # 2. Credenciales ScraperAPI
    api_key = "4cd72a5cadb77297cd9f41f11dc632c0"
    try:
        if "SCRAPERAPI_KEY" in st.secrets:
            api_key = st.secrets["SCRAPERAPI_KEY"]
    ...
    # Usamos proxy residencial de ScraperAPI para bypass de Cloudflare
    endpoint_proxy = f"https://api.scraperapi.com/?api_key={api_key}&url={quote(url_ripley, safe='')}&country_code=pe&ultra_premium=true"

    try:
        safe_log("🚀 [Ripley] Consultando catálogo vía ScraperAPI Ultra Proxy...", "info")
        resp = requests.get(endpoint_proxy, timeout=60)
        
        if resp.status_code == 200 and len(resp.text) > 5000:
            soup = BeautifulSoup(resp.text, 'html.parser')
            
            # Buscamos la data incrustada en __NEXT_DATA__
            next_script = soup.find('script', id='__NEXT_DATA__')
                ...
                try:
                    ...
                    for p in lista_acumulada:
                        try:
                            ...
                            link_final = f"https://simple.ripley.com.pe{link_rel}" if link_rel.startswith("/") else link_rel
            ...
            safe_log(f"✅ [Ripley] Respuesta recibida correctamente.", "info")
        else:
    ...
    productos_list = list(productos_map.values())
    if productos_list:
        safe_log(f"✅ [Ripley] ¡Éxito TOTAL! Se encontraron {len(productos_list)} ofertas.", "success")
    else:
