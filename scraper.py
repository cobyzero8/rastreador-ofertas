            ...
            if prods:
                for p in prods:
                    if p['precio'] <= limite:
                        ...
                        # ✅ CORRECCIÓN MAESTRA: Detección infalible de catálogos para IDs dinámicos
                        if "natura" in url_radar or "tiendabelcorp" in url_radar or "cyzone" in url_radar or "lbel" in url_radar or "esika" in url_radar:
                            # Creamos un identificador único basado en el nombre real del perfume escaneado
                            nombre_limpio = str(p['nombre']).upper().replace(" ", "_").replace("-", "_")
