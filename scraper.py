...
def escanear_tienda(url_base, limite_precio, tienda, talla_buscada, item_id):
    ...
    try:
        ...
        # MEJORA 5: Si la página arroja error de caída o descontinuado, se cataloga
        if respuesta.status_code != 200:
            try: supabase.table("radares").update({"url": "https://muerto_o_sin_stock"}).eq("id", item_id).execute()
            except: pass
            return []
...
# --- MEJORA 2: BARRENDERO DE CUPONES INTELIGENTE FILTRADO POR TU INTERÉS ---
def simular_rastreo_cupones_global(tiendas_usuario):
    ...
    # Filtrar el banco dejando solo las tiendas que el usuario tiene activas en Supabase
    ...
    if cupones_filtrados:
        txt_t = f"🎟️ *CUPONES FILTRADOS COBY & GEMINI*\n🤖 _Filtrado inteligente según tus intereses de hoy_\n———————————————————\n\n🔥 Se han detectado códigos válidos para tus radares activos. Revísalos en la pestaña *Cuponera Express* de tu web."
        try: requests.post(f"https://api.telegram.org/bot{TOKEN_TELEGRAM}/sendMessage", json={"chat_id": CHAT_ID_TELEGRAM, "text": txt_t, "parse_mode": "Markdown"})
        except: pass
...
def revisar_ofertas():
    ...
    # MEJORA 4: DETECTOR DE HORARIO DE MADRUGADA (Hora Perú UTC-5 aproximada)
    ...
    for item in lineas:
        if "muerto" in item["url"]: continue # Saltarse links muertos
        ...
        for p in productos:
            ...
            # MEJORA 1: EL GUARDIÁN HISTÓRICO (Detector de falsas ofertas)
            ...
            # Si el precio actual es mayor al promedio histórico, es una falsa oferta inflada
            if precios_anteriores and p['precio_descuento'] > (precio_promedio * 1.05):
                alert_estafa = "⚠️ *ALERTA:* _Falsa oferta detectada. El precio promedio histórico es menor._"
            else:
                alert_estafa = "✅ *OFERTA REAL RECOMENTADA*"

            historial[id_producto][fecha_hoy] = p['precio_descuento']
            
            # MEJORA 3: CALCULADORA DE AHORRO MENSUAL ACUMULADO
