...
st.sidebar.caption("🚀 _Central de Inteligencia Avanzada v9.9_")
...
if menu == "📈 Ver Dashboard":
    ...
    with tab3:
        # --- LÍNEA 88 COMPLETAMENTE SANEADA Y COMPRENSIBLE AQUÍ ---
...
# --- GESTIONAR ENLACES PRO ---
    ...
    if lineas:
        ...
        st.write("### 👁️‍gsub TABLA MAESTRA DE INSPECCIÓN ABSOLUTA (Busca duplicados aquí)")
        st.info("💡 Usa la lupa de la esquina derecha de la tabla para buscar palabras clave como 'ADIDAS' o 'ESIKA' y pillar si hay URLs repetidas.")
        ...
        st.download_button(label="📥 EXPORTAR TODA LA BASE DE DATOS A EXCEL/CSV", data=csv_data, file_name="base_datos_radares_completa.csv", mime="text/csv", use_container_width=True)
    else:
            ...
            if nombre and url:
                ...
                if not st.session_state.mod_url:
                    for item_db in lineas:
                        if item_db["url"].strip().lower() == url_limpia.lower():
                            ya_existe_url = True
                            id_repetido = item_db["identificador"]
                            break
                if ya_existe_url:
                    ...
                    supabase.table("radares").insert({"url": url_limpia, "precio_max": precio_max, "identificador": nuevo_id}).execute()
