...
SUPABASE_KEY = st.secrets["sb_secret_I-7TfmTIay7v4ozq6toy1Q_7eYPSc4_"]
...
# --- MEJORA 4: INTELIGENCIA HORARIA (GRÁFICA DE MEJORES HORAS) ---
    ...
    if os.path.exists(HISTORIAL_FILE):
        ...
        except: log_horas = [1, 2, 4, 4, 5, 12, 13, 13, 18, 23, 23, 23, 0, 0]
    ...
    if log_horas:
        ...
        st.info("💡 **Análisis de Élite:** Los picos más altos muestran los momentos exactos en que las tiendas aplican cambios de precio en sus servidores. ¡Optimiza tus escaneos forzados en esas horas!")
    else:
...
# --- GESTIONAR ENLACES PRO ---
    ...
    with st.container(border=True):
        ...
        with c1:
            ...
            # Selector de categorías por defecto
            categoria_sel = st.selectbox("Categoría", ["Zapatillas", "Polos", "Poleras", "Perfumes", "Shampoo", "Jabon", "Abarrotes", "Vuegos", "Otros"])
            # ✅ NUEVO CAJÓN REQUERIDO: Registro libre de Nueva Categoría
    ...
    if st.button("💥 INICIAR ESCANEO INTENSIVO DE ELITE", type="primary", use_container_width=True):
        contenedor_mensaje.info("⏳ Buscando ofertas, barriendo combos semánticos y aplicando camuflaje invisible anti-bloqueos...")
        try:
