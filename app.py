...
# CONFIGURACIÓN PREMIUM CON EL NOMBRE DE LA FIRMA
...
st.sidebar.caption("🚀 _Central de Inteligencia Familiar v5.0_")
st.sidebar.caption("⚡ Status: **Grandes Genios en Acción**")
st.sidebar.write("---")
                ...
                if os.path.exists(CUPONES_FILE):
                    ...
                    if lista_cupones_tabla:
                        ...
                        if st.button("🗑️ Vaciar Historial de Cupones Expirados", type="secondary"):
                            ...
                            st.rerun()
                    else: st.info("Aún no se han recolectado cupones globales en este ciclo.")
                else: st.info("Archivo de cuponera inicializado.")
    ...
    with st.container(border=True):
        ...
        if st.button("💾 GUARDAR CAMBIOS EN EL RADAR", type="primary", use_container_width=True):
            if nombre and url:
                ...
                lineas_actuales = []
                if os.path.exists(URLS_FILE):
                    with open(URLS_FILE, "r", encoding="utf-8") as f: lineas_actuales = [l.strip() for l in f.readlines() if l.strip()]
