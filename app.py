...
st.sidebar.caption("🚀 _Central de Inteligencia Avanzada v12.4_")
st.sidebar.caption("⚡ Sistema: **Diagnóstico de Emergencia Activo**")
st.sidebar.write("---")
...
# =========================================================================
    ...
    if os.path.exists(HISTORIAL_FILE):
        ...
        except Exception as e: st.error(f"❌ Error leyendo historial: {e}")
    ...
    if isinstance(logs, dict) and len(logs) > 0:
        ...
        except Exception as e: st.error(f"❌ Error pintando gráfica comercial: {e}")
    else:
...
# =========================================================================
    ...
    if os.path.exists(HISTORIAL_FILE):
        ...
        except Exception as e: st.error(f"❌ Error leyendo ahorros: {e}")
    ...
    with c2:
        ...
        try:
            ...
            st.bar_chart(df_sim)
        except Exception as e: st.error(f"❌ Error al cargar gráfica mensual: {e}")
...
# =========================================================================
    ...
    st.write("🔬 _Verificando estado de componentes..._")
    ...
    try:
        ...
        st.error(f"❌ No se pudo descargar datos de Supabase. Revisa la consola. Error: {e}")
        ...
        try:
            ...
            if not df_completo.empty:
                ...
                with st.expander("👁️‍gnd DESPLEGAR ACORDEÓN: VER TODOS MIS ENLACES GUARDADOS EN LA NUBE", expanded=False):
                    st.write("### 📋 Vista de Auditoría Completa")
            ...
            st.warning(f"⚠️ No se pudo armar la tabla del acordeón, pero puedes editar abajo. Detalles: {e}")
    else:
            ...
            if nombre and url:
                ...
                if ya_existe:
                    ...
                    try:
                        ...
                        st.rerun()
                    except Exception as e: st.error(f"❌ Error al insertar en Supabase: {e}")
