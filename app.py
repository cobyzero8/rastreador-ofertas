...
# ==========================================
    ...
    categorias_formulario = ["Perfumes", "Zapatillas", "Ropa (Medias)", "Ropa (Polos)", "Ropa (Casacas/Poleras)", "Ropa (Shorts)", "Ropa (Buzos)", "Ropa (Deportivos)", "Tecnologia"]
    ...
    with st.container(border=True):
        ...
        with c1:
            ...
            tienda_sel = st.selectbox("Selecciona Tienda", lista_tiendas, index=idx_tnd)
            
            idx_cat = categorias_formulario.index(st.session_state.mod_cat) if st.session_state.mod_cat in categorias_formulario else 0
            cat_menu = st.selectbox("Categoría Principal", categorias_formulario, index=idx_cat)
        with c2:
                ...
                with st.container(border=True):
                    ...
                    with col_mod:
                        ...
                        if st.button(f"📝 Modificar", key=f"mod_btn_{item['id']}", use_container_width=True):
                            ...
                            st.session_state.mod_cat = rev_mapa.get(cat_p, "Zapatillas")
