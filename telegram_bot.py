...
# ---------------------------------------------------------
# Listas Oficiales de Tiendas y Categorías
# ---------------------------------------------------------
TIENDAS = [
...
# ---------------------------------------------------------
# Controladores de Mensajes y Menús
# ---------------------------------------------------------
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🤖 *Central de Control - Cazador de Ofertas*\n\nSelecciona una opción interactiva o usa los comandos directos en el chat:",
...
async def ejecutar_escaneo(update: Update, context: ContextTypes.DEFAULT_TYPE, filtro: str):
    ...
    if query:
        await query.answer()
        await query.edit_message_text(f"🔍 Escaneando filtro: *{filtro}*...", parse_mode="Markdown")
        chat_id = query.message.chat_id
    else:
        await update.message.reply_text(f"🔍 Escaneando filtro: *{filtro}*...", parse_mode="Markdown")
        chat_id = update.effective_chat.id
        
    resumen = revisar_ofertas(filtro)
    await context.bot.send_message(
        chat_id=chat_id, 
        text=f"✅ *Escaneo [{filtro}] finalizado:*\n{resumen}", 
        parse_mode="Markdown"
    )
...
def main():
    ...
    # Registrar comandos automáticos para cada Tienda (/tienda_ripley, /tienda_nike, etc.)
    for tienda in TIENDAS:
        cmd = f"tienda_{tienda.lower()}"
        app.add_handler(CommandHandler(cmd, lambda u, c, t=tienda: ejecutar_escaneo(u, c, t)))
        
    # Registrar comandos automáticos para cada Categoría (/cat_zapatillas, /cat_tv, etc.)
